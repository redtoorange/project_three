...
--
-- Body for the Fruit_Tree package
--
...
-- Public Subroutines ---------------------------------------------------------
   ...
   begin
      ...
      while not End_Of_File (input_file) loop
         ...
         -- Inner Exception Handling to allow recovery from bad file data
         begin
            ...
            if next_key = TREE then
               ...
               if last_index /= 0 then
                  ...
                  Put_Line (": Cannot add fruit, no parent Tree.");
               end if;
            end if;
   ...
   begin
      ...
      -- Ensure the next character was a space or newline
...
-- Private Subroutines --------------------------------------------------------
   ...
   begin
      ...
      for f of t.fruits (1 .. t.f_count) loop
         sum := sum + Get_Fruit_Value (qual, f);
      end loop;
   ...
   begin
      ...
      for f of t.fruits (1 .. t.f_count) loop
         sum := sum + ((Get_Fruit_Value (qual, f) - average)**2);
      end loop;
